...
interface Pedido { id: string; numero_pedido: number; codigo_retiro: string; estado: string; total: number; metodo_pago: string | null; origen: string; notas: string | null; created_at: string; sucursales?: { nombre: string }; pedido_items: PedidoItem[] }

const ESTADO_LABEL: Record<string, string> = { PENDING_PAYMENT: 'PENDIENTE PAGO', PAID: 'PAGADO', PREPARING: 'EN PREPARACIÓN', READY: 'LISTO' }
const ESTADO_COLOR: Record<string, string> = { PENDING_PAYMENT: 'bg-red-100 text-red-700', PAID: 'bg-blue-100 text-blue-700', PREPARING: 'bg-amber-100 text-amber-700', READY: 'bg-green-100 text-green-700' }
const ESTADO_BORDER: Record<string, string> = { PENDING_PAYMENT: 'border-l-red-400', PAID: 'border-l-blue-400', PREPARING: 'border-l-amber-400', READY: 'border-l-green-400' }
...
export default function VistaCaja({ dispositivo, sesion }: { dispositivo: Dispositivo; sesion: SesionOperador }) {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`caja-${dispositivo.sucursal_id}`)
      .on('postgres_changes', {
        event: '*',
        schema: 'public',
        table: 'pedidos',
  ...
  // Actualizar seleccionado cuando cambian los pedidos
  useEffect(() => {
    if (seleccionado) {
  ...
  const counts = tabsDef.reduce((acc, t) => ({ ...acc, [t.key]: pedidos.filter(p => p.estado === t.key).length }), {} as Record<string, number>)
      ...
      <div className="flex bg-white border-b border-neutral-200 px-4 gap-1">
        ...
        {tabsDef.map(t => counts[t.key] > 0 && (
          <button key={t.key} onClick={() => { setTab('activos'); setFiltroEstado(t.key) }}
            ...
            {t.label} <span className={`text-xs font-bold px-1.5 py-0.5 rounded-full bg-neutral-100 ${t.color}`}>{counts[t.key]}</span>
          </button>
        ))}
        ...
        <div className="flex-1 flex overflow-hidden">
          <div className="w-72 border-r border-neutral-200 flex flex-col overflow-hidden bg-neutral-50">
            <div className="flex items-center justify-between px-3 py-2 border-b border-neutral-200">
              ...
              <button onClick={cargarPedidos} className="p-1 text-neutral-400 hover:text-neutral-600"><RefreshCw className="h-3.5 w-3.5" /></button>
            </div>
            <div className="flex-1 overflow-y-auto">
              {loading ? (
                ...
                <button key={pedido.id} onClick={() => setSeleccionado(pedido)}
                  ...
                  <p className="text-neutral-500 text-xs truncate">{pedido.pedido_items.map(i => i.nombre_producto_snap).join(', ')}</p>
            ...
            {!seleccionado ? (
              ...
              <div className="max-w-lg">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <span className={`px-3 py-1.5 rounded-lg text-sm font-bold ${ESTADO_COLOR[seleccionado.estado]}`}>{ESTADO_LABEL[seleccionado.estado]}</span>
                </div>
                          ...
                          {item.pedido_item_opciones.length > 0 && (
                            <div className="flex flex-wrap gap-1 mt-1.5">
                              {item.pedido_item_opciones.map((op, j) => (
                                <span key={j} className="text-xs bg-neutral-100 text-neutral-600 px-2 py-0.5 rounded-full">{op.emoji_snap} {op.nombre_snap}</span>
                              ))}
                            </div>
                          )}
                        </div>
