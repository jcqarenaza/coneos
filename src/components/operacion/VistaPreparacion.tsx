...
export default function VistaPreparacion({ dispositivo, sesion }: { dispositivo: Dispositivo; sesion: SesionOperador }) {
  ...
  return (
    <div className="h-full flex overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-4">
        ...
        <div className="flex gap-3 mb-5">
          ...
          <div className="flex-1 bg-amber-50 rounded-xl border border-amber-200 p-3 text-center">
            <p className="text-2xl font-bold text-amber-700">{stats.preparando}</p>
        ...
        {pedidos.length === 0 ? (
          ...
          <div className="space-y-4">
            {nuevos.length > 0 && (
              <div>
                <p className="text-xs font-semibold text-neutral-400 uppercase tracking-wide mb-2">Por preparar ({nuevos.length})</p>
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
                  {nuevos.map(pedido => (
                    <div key={pedido.id}
                      onClick={() => setSeleccionado(seleccionado?.id === pedido.id ? null : pedido)}
                      ...
                      <div className="bg-blue-50 px-4 py-2.5 flex items-center justify-between border-b border-blue-100">
                        ...
                        <div className="flex items-center gap-1 text-blue-400 text-xs">
                          ...
                          {tiempoRelativo(pedido.created_at)}
                        </div>
                      </div>
                      ...
                      {verTodas && pedido.sucursales?.nombre && (
                        <div className="px-3 pb-2"><span className="text-xs text-neutral-400">📍 {pedido.sucursales.nombre}</span></div>
                      )}
                      <div className="px-3 pb-3">
                        <button onClick={e => { e.stopPropagation(); cambiarEstado(pedido.id, 'PREPARING') }}
            ...
            {enPreparacion.length > 0 && (
              <div>
                <p className="text-xs font-semibold text-neutral-400 uppercase tracking-wide mb-2">En preparación ({enPreparacion.length})</p>
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
                  {enPreparacion.map(pedido => (
                    <div key={pedido.id}
                      ...
                      <div className="bg-amber-50 px-4 py-2.5 flex items-center justify-between border-b border-amber-100">
                        ...
                        <div className="flex items-center gap-2">
                          {verTodas && pedido.sucursales?.nombre && (
                            <span className="text-xs bg-white px-1.5 py-0.5 rounded text-neutral-500">{pedido.sucursales.nombre}</span>
                          )}
                      ...
                      <div className="px-3 pb-3">
                        <button onClick={e => { e.stopPropagation(); cambiarEstado(pedido.id, 'READY') }}
                          disabled={procesando === pedido.id}
      ...
      {/* Panel derecho — detalle seleccionado */}
      {seleccionado && (
