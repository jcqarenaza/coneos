...
export default function VistaCaja({ dispositivo, sesion }: { dispositivo: Dispositivo; sesion: SesionOperador }) {
  ...
  const cargarPedidos = useCallback(async () => {
    ...
    // Si tiene sucursal específica, filtrar — si es null ve todas
    if (!verTodas) {
    ...
    <div className="h-full flex flex-col">
      <div className="flex bg-white border-b border-neutral-200 px-4">
        <button onClick={() => setTab('activos')}
          className={`flex items-center gap-2 px-4 py-3 text-sm font-medium border-b-2 transition-colors ${tab === 'activos' ? 'border-neutral-800 text-neutral-900' : 'border-transparent text-neutral-400 hover:text-neutral-600'}`}>
                ...
                return (
                  <div key={pedido.id} className="bg-white rounded-xl border border-neutral-200 shadow-sm overflow-hidden">
                    <div className={`px-4 py-2.5 flex items-center justify-between border-b ${cfg?.bg}`}>
                      ...
                      <div className="flex items-center gap-2">
                        {verTodas && pedido.sucursales?.nombre && (
                          <span className="text-xs bg-white/60 px-2 py-0.5 rounded-full text-neutral-500 font-medium">{pedido.sucursales.nombre}</span>
                        )}
                    ...
                    <div className="p-4 space-y-2.5">
                      ...
                      {pedido.notas && <p className="text-amber-600 text-xs border-t border-neutral-100 pt-2">📝 {pedido.notas}</p>}
                    </div>
                    <div className="px-4 pb-4 flex items-center justify-between">
                      <p className="text-neutral-800 font-bold">{formatPrecio(pedido.total)}</p>
                      {SIGUIENTE_ESTADO[pedido.estado] && (
                        <button onClick={() => cambiarEstado(pedido.id, pedido.estado)} disabled={procesando === pedido.id}
                          ...
                          {procesando === pedido.id ? <Loader2 className="h-4 w-4 animate-spin" /> : LABEL_ACCION[pedido.estado]}
                        </button>
                      )}
                    </div>
                  </div>
