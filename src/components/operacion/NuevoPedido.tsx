...
interface Opcion { id: string; nombre: string; emoji: string | null; color: string | null }
interface ItemCarrito { presentacion: Presentacion; producto: Producto; opciones: Opcion[]; cantidad: number }
...
export default function NuevoPedido({ dispositivo, sesion, onPedidoCreado }: Props) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      supabase.from('opciones').select('id, nombre, emoji, color').eq('empresa_id', dispositivo.empresa_id).eq('activo', true).is('deleted_at', null).order('orden'),
    ]).then(([cats, prods, pres, ops]) => {
  ...
  function seleccionarPresentacion(p: Presentacion, prod: Producto) { setPresentacionActiva(p); setProductoActivo(prod); setOpcionesSeleccionadas([]) }
  ...
  async function confirmarPedido() {
    ...
    setGuardando(false)
    if (res.ok) { setExito(true); setCarrito([]); setNotas(''); setTimeout(() => { setExito(false); onPedidoCreado() }, 1500) }
  }
  ...
  return (
    <div className="flex h-full overflow-hidden">
              ...
              {productosFiltrados.map(prod => {
                ...
                return pres.map(p => (
                  <button key={p.id} onClick={() => seleccionarPresentacion(p, prod)}
                    className="flex flex-col items-start p-4 bg-white rounded-2xl border border-neutral-100 hover:border-neutral-200 hover:shadow-sm transition-all text-left active:scale-98">
                    <p className="text-neutral-800 font-bold text-sm">{prod.nombre}</p>
                    <p className="text-neutral-400 text-xs mt-0.5">{p.nombre}</p>
                    <p className="text-neutral-700 font-black mt-2 text-base">{formatPrecio(p.precio)}</p>
                  </button>
                ))
              })}
            </div>
      ...
      {/* Carrito */}
      <div className="w-72 border-l border-neutral-100 bg-white flex flex-col shadow-sm">
            ...
            <div key={i} className="bg-neutral-50 rounded-xl p-3">
              <div className="flex items-start justify-between gap-2">
                <div className="flex-1">
                  <p className="text-neutral-700 text-sm font-bold">{item.producto.nombre}</p>
                ...
                <div className="flex items-center gap-1.5">
                  <p className="text-neutral-700 text-sm font-bold">{formatPrecio(item.presentacion.precio)}</p>
                  <button onClick={() => setCarrito(prev => prev.filter((_, idx) => idx !== i))} className="text-neutral-200 hover:text-red-400 transition-colors">
