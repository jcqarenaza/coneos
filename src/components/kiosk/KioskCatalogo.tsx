...
export default function KioskCatalogo({ dispositivo, config, carrito, categoriaIdInicial, onAgregar, onVerCarrito, onVolver }: Props) {
  ...
  function volverPaso() {
    if (paso === 'opciones') {
      ...
      else { setPaso('presentacion'); setCola([]); setColaIndex(0); setOpcionesSeleccionadas([]) }
              ...
              {productosFiltrados.map(prod => (
                <button key={prod.id} onClick={() => seleccionarProducto(prod)}
                  ...
                  <div className="p-4">
                    ...
                    {prod.descripcion && <p className="text-neutral-400 text-sm mt-1">{prod.descripcion}</p>}
