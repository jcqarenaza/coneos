    ...
    <div className="min-h-screen flex flex-col" style={{ backgroundColor: '#fdf8f4' }}>
        ...
        <div className="flex items-center justify-center">
          {config.logo_url ? (
            <div className="bg-white rounded-xl px-5 py-2 flex items-center justify-center" style={{ minWidth: 160, maxWidth: 220 }}>
              <Image src={config.logo_url} alt={dispositivo.empresas?.nombre ?? 'Logo'} width={180} height={65} className="object-contain w-auto" style={{ maxHeight: 56 }} />
            </div>
          ) : (
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-8 py-10">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 w-full max-w-3xl">
          {categorias.map(cat => (
            <button key={cat.id} onClick={() => onComenzar(cat.id)}
