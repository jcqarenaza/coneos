...
export async function GET(request: Request) {
  ...
  const [{ data: categorias }, { data: productos }, { data: presentaciones }, { data: grupos }, { data: opciones }, { data: inventario }, { data: presGrupos }, { data: catalogoConfig }] =
    await Promise.all([
      ...
      supabase.from('presentaciones').select('id, nombre, precio, permite_opciones, opciones_min, opciones_max, orden, producto_id, imagen_url').eq('empresa_id', empresa_id).eq('activo', true).eq('visible_kiosk', true).order('orden'),
  ...
  const productosFiltrados = (productos ?? []).filter((p: { id: string }) => dispoMap[p.id] !== false)
  const presentacionesFiltradas = (presentaciones ?? []).filter((p: { id: string }) => dispoMap[p.id] !== false)
