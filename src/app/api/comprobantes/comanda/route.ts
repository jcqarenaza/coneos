...
export async function POST(request: Request) {
  ...
  const { data: pedido } = await supabase
    .from('pedidos')
    .select(`id, numero_pedido, total, metodo_pago, datos_delivery, costo_envio, colaborador_nombre, created_at, tipo_pedido, hora_retiro,
  ...
  // Accesorios agrupados bajo un solo título (sin precios: la comanda es para preparar)
  ...
  if (accesorios.length > 0) {
    bloquesItems.push(`<div><div class="item-prod">Accesorios</div>${accesorios.map(item => `<div class="item-pres">${item.cantidad > 1 ? `${item.cantidad}x ` : ''}${item.nombre_presentacion_snap.replace(/^Toppings?\s+/i, '')}</div>`).join('')}</div>`)
  }
