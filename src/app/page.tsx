...
export default function LandingPage() {
  return (
    <div className="min-h-screen font-sans text-white" style={{ background: 'linear-gradient(165deg, #081A2E, #0D2C4F 55%, #0F3358)' }}>
      ...
      <nav className="border-b border-white/10 px-6 md:px-8 py-4 flex items-center justify-between sticky top-0 bg-[#0A1F35]/95 backdrop-blur-sm z-50">
        <div className="flex items-center gap-2.5">
          ...
          <span className="font-bold text-lg border-l border-white/15 pl-2.5">Gastr<span style={{ color: AZUL }}>OS</span></span>
        </div>
        <div className="flex items-center gap-5">
          <a href="#funciones" className="text-slate-400 text-sm hover:text-white transition-colors hidden md:block">Funcionalidades</a>
          <a href="#rubros" className="text-slate-400 text-sm hover:text-white transition-colors hidden md:block">Soluciones</a>
          <a href="#precios" className="text-slate-400 text-sm hover:text-white transition-colors hidden md:block">Precios</a>
          <a href="#faq" className="text-slate-400 text-sm hover:text-white transition-colors hidden md:block">FAQ</a>
      ...
      <section className="px-6 md:px-8 pt-16 pb-20 overflow-hidden" style={{ background: 'linear-gradient(165deg, #081A2E, #0D2C4F 55%, #0F3358)' }}>
      ...
      {/* BARRA DE CONFIANZA */}
      <section className="px-6 md:px-8 py-10 border-y border-white/10 bg-white/[.04]">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <p className="text-slate-400 text-xs font-semibold tracking-widest">MESAS · DELIVERY · PAGOS · FISCAL · CLIENTES · REPORTES</p>
        </div>
      </section>
      ...
      <section className="px-6 md:px-8 py-20 max-w-3xl mx-auto text-center">
        ...
        <div className="flex flex-wrap justify-center gap-2 mb-6 text-sm text-slate-400">
          {['Pedidos por WhatsApp', 'Mesas por un lado', 'Caja por otro', 'Delivery manual', 'Comandas impresas', 'Pagos difíciles de controlar'].map(p => (
            <span key={p} className="px-3 py-1.5 bg-white/[.04] border border-white/10 rounded-full">{p}</span>
          ))}
        </div>
      ...
      {/* FUNCIONALIDADES */}
      <section id="funciones" className="px-6 md:px-8 py-20 bg-white/[.04]">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {MODULOS.map(m => (
              <div key={m.title} className="bg-white/[.06] rounded-2xl border border-white/10 p-6 border-t-4 transition-shadow hover:shadow-md" style={{ borderTopColor: m.color }}>
      ...
      <section id="rubros" className="px-6 md:px-8 py-20 max-w-4xl mx-auto text-center">
        ...
        <p className="text-slate-400 text-lg mb-10">Una misma plataforma, configurable. No son productos distintos: es tu catálogo y tu operación, con tus nombres.</p>
        <div className="flex flex-wrap justify-center gap-3">
          {RUBROS.map(([emoji, nombre], i) => (
            <span key={nombre} className="flex items-center gap-2 px-4 py-2.5 bg-white/[.06] border border-white/10 rounded-2xl text-sm font-semibold shadow-sm border-b-2" style={{ borderBottomColor: [AZUL, VERDE, AMARILLO, ROJO, NAVY][i % 5] }}>
              <span className="text-xl">{emoji}</span>{nombre}
            </span>
          ))}
        </div>
        <p className="text-slate-400 text-sm mt-8">GastrOS adapta su catálogo y operación a la forma en que trabaja cada negocio.</p>
      </section>
      ...
      <section className="px-6 md:px-8 py-20 max-w-4xl mx-auto text-center">
        ...
        <p className="text-slate-400 text-lg mb-8">Porque en gastronomía cada segundo cuenta.</p>
        <div className="flex flex-wrap justify-center gap-2 mb-10 text-sm">
          {['Interfaz táctil', 'Celular', 'Tablet', 'Tótem', 'Pantallas horizontales', 'Responsive', 'Instalable como app (PWA)', 'Operación simple'].map(t => (
            <span key={t} className="px-3.5 py-2 bg-white/[.04] border border-white/10 rounded-full font-semibold text-slate-300">{t}</span>
          ))}
        </div>
      ...
      {/* ADMINISTRACIÓN */}
      <section className="px-6 md:px-8 py-20 bg-white/[.04]">
        <div className="max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
          <div>
            <h2 className="text-3xl md:text-4xl font-black mb-4">Vos controlás el negocio.</h2>
            <p className="text-slate-400 mb-6 leading-relaxed">Toda la configuración de tu negocio desde un único lugar.</p>
            <div className="flex flex-wrap gap-2 text-xs font-semibold text-slate-300">
              {['Ventas', 'Productos', 'Categorías', 'Sucursales', 'Usuarios', 'Dispositivos', 'Mesas', 'Beneficios', 'Facturación', 'Configuración'].map(t => (
                <span key={t} className="px-3 py-1.5 bg-white/[.06] border border-white/10 rounded-full">{t}</span>
              ))}
            </div>
          </div>
          <div className="bg-white/[.06] rounded-3xl border border-white/10 shadow-sm overflow-hidden">
            <div className="border-b border-white/10 px-4 py-2.5 flex items-center gap-2">
              <img src="/aoki-logo.png" alt="" className="h-4 w-auto" />
            ...
            <div className="p-4 grid grid-cols-2 gap-3">
              {([['Ventas de hoy', '$486.000', VERDE], ['Pedidos', '92', AZUL], ['Ticket promedio', '$5.280', AMARILLO], ['Por canal', 'K 41 · D 32 · M 19', ROJO]] as [string, string, string][]).map(([l, v, c]) => (
                <div key={l} className="rounded-2xl border border-white/10 p-3">
      ...
      <section className="px-6 md:px-8 py-20 max-w-4xl mx-auto text-center">
        ...
        <p className="text-slate-400 text-lg mb-10 max-w-2xl mx-auto">GastrOS está preparado para trabajar con múltiples sucursales, manteniendo la operación y la información de cada una organizada.</p>
        <div className="flex flex-col items-center gap-3">
          ...
          <div className="flex flex-wrap justify-center gap-3">
            {['Sucursal 1', 'Sucursal 2', 'Sucursal 3', 'Sucursal 4'].map((s, i) => (
              <div key={s} className="px-5 py-3 bg-white/[.06] border border-white/10 rounded-2xl text-sm font-semibold shadow-sm border-t-4" style={{ borderTopColor: [AZUL, VERDE, AMARILLO, ROJO][i] }}>🏪 {s}</div>
            ))}
          </div>
        </div>
      </section>
        ...
        <div className="max-w-4xl mx-auto text-center">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
            {([['🔐', 'Datos separados por comercio'], ['👤', 'Usuarios y permisos'], ['🔑', 'Acceso seguro'], ['☁️', 'Infraestructura cloud'], ['💾', 'Copias de seguridad'], ['🤝', 'Integraciones seguras con pagos y servicios fiscales']] as [string, string][]).map(([e, t]) => (
              <div key={t} className="bg-white/[.06] rounded-2xl border border-white/10 p-5">
      ...
      <section className="px-6 md:px-8 py-20 max-w-4xl mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
          ...
          <div className="bg-white/[.06] border border-white/10 rounded-3xl p-6 shadow-sm font-mono text-xs leading-relaxed max-w-xs mx-auto w-full">
            <p className="text-center font-black">TU COMERCIO</p>
            <p className="text-center text-[10px] text-slate-400 border-b border-dashed border-white/20 pb-2 mb-2">FACTURA · 00003-00000116</p>
            <div className="flex justify-between"><span>Almuerzo x4</span><span>$48.000</span></div>
            <div className="flex justify-between font-black border-t border-dashed border-white/20 mt-2 pt-2"><span>TOTAL</span><span>$48.000</span></div>
            <p className="text-[10px] text-slate-400 mt-2">CAE: 86361990251606</p>
            <div className="w-16 h-16 mx-auto mt-2 rounded-lg border border-white/15 grid grid-cols-4 grid-rows-4 gap-0.5 p-1">
      ...
      {/* PAGOS */}
      <section className="px-6 md:px-8 py-16 bg-white/[.04]">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <div className="flex flex-wrap justify-center gap-4">
            {([['💳', 'Mercado Pago', 'con la cuenta del comercio'], ['🏦', 'Transferencia', 'con alias copiable'], ['💵', 'Efectivo', 'con control en caja']] as [string, string, string][]).map(([e, t, d]) => (
              <div key={t} className="bg-white/[.06] rounded-2xl border border-white/10 px-6 py-5 shadow-sm">
      ...
      <section className="px-6 md:px-8 py-20 max-w-4xl mx-auto text-center">
        ...
        <p className="text-slate-400 text-lg mb-12">Vos conocés tu negocio. Nosotros nos encargamos de convertirlo en una operación conectada.</p>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5 text-left">
          {([['01', AZUL, 'Configuramos tu negocio', 'Productos, precios, categorías, sucursal y dispositivos.'], ['02', VERDE, 'Ponemos GastrOS a funcionar', 'Kiosco, mesas, delivery, caja y operación según corresponda.'], ['03', AMARILLO, 'Te acompañamos', 'Capacitación y puesta en marcha, con soporte cercano.']] as [string, string, string, string][]).map(([n, c, t, d]) => (
            <div key={n} className="bg-white/[.06] rounded-2xl border border-white/10 p-6 shadow-sm">
      ...
      <section id="precios" className="px-6 md:px-8 py-20 bg-white/[.04]">
        <div className="max-w-5xl mx-auto text-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-stretch text-left">
            {LANDING.precios.planes.map(plan => (
              <div key={plan.id}
                className={`bg-white/[.06] rounded-3xl p-7 shadow-sm border-t-4 relative flex flex-col ${plan.destacado ? 'border border-white/15 shadow-md md:-translate-y-2' : 'border border-white/10'}`}
                ...
                <p className="text-slate-400 text-sm mb-4">{plan.bajada}</p>
                <p className="text-2xl font-black mb-0.5">{plan.precio}</p>
                <p className="text-slate-400 text-xs mb-5">{plan.detalle}</p>
                {plan.base && <p className="text-xs font-bold text-slate-400 uppercase tracking-wide mb-2">{plan.base}</p>}
                ...
                <a href="#contacto" className={`mt-6 text-center text-sm font-bold rounded-xl py-3 transition-colors ${plan.destacado ? 'text-white' : 'border border-white/15 text-slate-300 hover:bg-white/10'}`}
                  style={plan.destacado ? { backgroundColor: plan.color } : undefined}>Quiero este plan</a>
              </div>
            ))}
          </div>
          <p className="text-slate-400 text-sm mt-8 font-semibold">{LANDING.precios.mensualidad}</p>
      ...
      <section id="faq" className="px-6 md:px-8 py-20 max-w-2xl mx-auto">
        ...
        <div className="space-y-3">
          {FAQ.map(([q, a]) => (
            <details key={q} className="group bg-white/[.06] border border-white/10 rounded-2xl px-5 py-4 shadow-sm">
