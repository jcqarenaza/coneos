...
export default function CatalogoPage() {
  ...
  // Presentaciones
  function openNewPres(prodId: string) { setFormPres({ nombre: '', precio: 0, permite_opciones: false, opciones_min: 0, opciones_max: 0, orden: 1, activo: true, producto_id: prodId, imagen_url: null, visible_kiosk: true, es_novedad: false }); setGruposSeleccionados([]); setEditId(null); setModalPres(true) }
  function openEditPres(p: Presentacion) { setFormPres({ nombre: p.nombre, precio: p.precio, permite_opciones: p.permite_opciones, opciones_min: p.opciones_min, opciones_max: p.opciones_max, orden: p.orden, activo: p.activo, producto_id: p.producto_id, imagen_url: p.imagen_url, visible_kiosk: p.visible_kiosk, es_novedad: p.es_novedad ?? false }); setGruposSeleccionados(presGrupos.filter(pg => pg.presentacion_id === p.id).map(pg => pg.grupo_id)); setEditId(p.id); setModalPres(true) }
  ...
  return (
    <div>
                        ...
                        <div key={prod.id} className={pi < catProds.length - 1 ? 'border-b border-neutral-50' : ''}>
                          ...
                          {(expandidos.has(prod.id) || busqueda.trim().length > 0) && (() => {
                            ...
                            const saboresProd = [...opciones]
                              .filter(op => grupoIds.has(op.grupo_id))
                              .sort((a, b) => a.nombre.localeCompare(b.nombre, 'es'))
                            if (saboresProd.length === 0) return null
                            return (
                                  ...
                                  {saboresProd.map(op => {
                                    ...
                                    return (
                                      <button
                                        ...
                                        className={`flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold border transition-all disabled:opacity-50 ${
                                          activo
                                        ...
                                        {op.nombre}
                                        {savingDispo === op.id && <Loader2 className="h-2.5 w-2.5 animate-spin" />}
                                      </button>
                                    )
                                  })}
                                </div>
      ...
      <ConeModal open={modalProd} onClose={() => setModalProd(false)} title={editId ? 'Editar producto' : 'Nuevo producto'}
        ...
        {(() => { const ca = categorias.find(c => c.id === formProd.categoria_id)
          return <div className="mb-4 -mt-1 text-xs font-semibold text-neutral-400 bg-neutral-50 border border-neutral-100 rounded-lg px-3 py-2">🌳 <span className="text-neutral-700">{ca?.nombre ?? 'elegí la categoría'}</span> <span className="text-neutral-300">›</span> <span className="text-blue-600">{editId ? 'este producto' : 'nuevo producto'}</span> <span className="text-neutral-300">› después: su presentación y precio</span></div> })()}
        <div className="space-y-4">
      ...
      <ConeModal open={modalPres} onClose={() => setModalPres(false)} title={editId ? 'Editar presentación' : 'Nueva presentación'}
        ...
        {(() => { const pr = productos.find(p => p.id === formPres.producto_id); const ca = categorias.find(c => c.id === pr?.categoria_id)
          return pr ? <div className="mb-4 -mt-1 text-xs font-semibold text-neutral-400 bg-neutral-50 border border-neutral-100 rounded-lg px-3 py-2">🌳 {ca?.nombre ?? '…'} <span className="text-neutral-300">›</span> <span className="text-neutral-700">{pr.nombre}</span> <span className="text-neutral-300">›</span> <span className="text-blue-600">{editId ? 'esta presentación' : 'nueva presentación'}</span></div> : null })()}
        <div className="space-y-4">
          ...
          <label className="flex items-center gap-2 cursor-pointer">
            ...
            <span className="text-sm text-neutral-700">⭐ Es novedad <span className="text-neutral-400">(aparece en el banner del kiosk y delivery)</span></span>
          </label>
          <label className="flex items-center gap-2 cursor-pointer">
