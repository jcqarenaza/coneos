// ═══════════════════════════════════════════════════════════════════
// arca-facturar — Edge Function ConeOS (multi-tenant) — FA-2 + MULTI-CUIT B2/B3
...
// ── MULTI-CUIT (JC 25/09) ──
// B2: el caller puede mandar facturacion_config_id (el emisor resuelto por la
//     cuenta del pedido). La Edge JAMÁS confía en él a ciegas: lo carga
//     verificando empresa + estado='validado' (y activo en la rama facturar).
//     Sin el parámetro: comportamiento BYTE-IDÉNTICO al histórico (fallback
//     sucursal→marca, ahora explícitamente limitado a configs es_fallback).
//     Toda factura/NC nueva CONGELA su emisor en facturas.facturacion_config_id.
// B3: la NC recupera el emisor DE SU FACTURA ORIGINAL (jamás re-resuelve).
//     Original histórica sin emisor congelado (null) → fallback documentado.
// ═══════════════════════════════════════════════════════════════════
...
// ── Config multi-tenant ──
// B2: TRES caminos de carga, en este orden de autoridad:
//   ① configId explícito (el emisor resuelto por la cuenta) → verificado:
//      empresa + estado='validado'. Jamás se confía en el id a ciegas.
//   ② sucursal del pedido → solo configs es_fallback=true
//   ③ marca (sucursal null) → solo configs es_fallback=true
// Los caminos ② y ③ son el comportamiento histórico byte-idéntico (la fila
// histórica de cada cliente es es_fallback=true por default de B0).
interface FactConfig {
  id: string; empresa_id: string; cuit: string; razon_social: string;
  ...
  emite_factura_a: boolean; estado: string;
...
async function getConfig(empresaId: string, sucursalId?: string | null, configId?: string | null): Promise<FactConfig> {
  ...
  if (!data && sucursalId) {
    const r = await supabase.from('facturacion_config')
      .select(SELECT_CFG).eq('empresa_id', empresaId).eq('sucursal_id', sucursalId)
      .eq('es_fallback', true).maybeSingle();
    data = r.data;
  }
  if (!data) {
    const r = await supabase.from('facturacion_config')
      .select(SELECT_CFG).eq('empresa_id', empresaId).is('sucursal_id', null)
      .eq('es_fallback', true).maybeSingle();
    data = r.data;
  }
...
interface BodyParams {
  ...
  motivo?: string;
  facturacion_config_id?: string; // B2: emisor resuelto por el caller (se RE-VERIFICA acá)
}
...
Deno.serve(async (req: Request) => {
  ...
  if (p.accion === 'nota_credito') {
    ...
    // B3: se recupera también su EMISOR CONGELADO.
    const { data: fOriginal } = await supabase.from('facturas')
      .select('id, tipo_cbte, punto_venta, nro_cbte, total, doc_tipo, doc_nro, cond_iva_receptor, imp_neto, imp_iva, facturacion_config_id')
    ...
    // ── B3: LA NC EMITE CON EL EMISOR DE SU FACTURA ORIGINAL — jamás
    // re-resuelve. Solo una original histórica (emisor null) usa el cfg del
    // fallback ya resuelto arriba (comportamiento documentado C6). No se exige
    // activo: anular siempre es posible (mismo criterio que nc-route).
    if (fOriginal.facturacion_config_id && fOriginal.facturacion_config_id !== cfg.id) {
      try {
        const r = await supabase.from('facturacion_config')
          .select(SELECT_CFG).eq('id', fOriginal.facturacion_config_id)
          .eq('empresa_id', p.empresa_id).maybeSingle();
        if (!r.data) return json({ok:false,error:'El emisor de la factura original ya no existe'},409);
        if (!r.data.cert_pem || !r.data.key_pem) return json({ok:false,error:'El emisor de la factura original no tiene certificado cargado'},409);
        cfg = { ...(r.data as object), emite_factura_a: !!r.data.emite_factura_a, scope_sucursal: (r.data.sucursal_id as string | null) ?? null } as FactConfig;
        const errCoh2 = validarCoherencia(cfg);
        if (errCoh2) return json({ ok: false, error: errCoh2 }, 400);
      } catch(err) {
    ...
    const { data: fRow, error: insertErr } = await supabase.from('facturas')
      .insert({ empresa_id: p.empresa_id, pedido_id: p.pedido_id, tipo_cbte: tipoCbte,
        ...
        facturacion_config_id: fOriginal.facturacion_config_id ?? cfg.id, // B3: la NC congela el emisor de su original
  ...
  const { data: fRow, error: insertErr } = await supabase.from('facturas')
    .insert({ empresa_id: p.empresa_id, pedido_id: p.pedido_id, tipo_cbte: tipoCbte,
      sucursal_id: sucursalPedido,
      facturacion_config_id: cfg.id, // B2: la factura CONGELA su emisor — jamás vuelve a preguntarlo
      punto_venta: cfg.punto_venta, total: impTotal, doc_tipo: docTipo, doc_nro: docNro,
      cond_iva_receptor: condIva, ambiente: cfg.ambiente, estado: 'pendiente',
      imp_neto: importes.conIva ? importes.neto : null,
      imp_iva: importes.conIva ? importes.iva : null })
